...
def base_load(df,capacity,power,diesel_size,capacity_cost,strict_baseload=2000,relaxed_baseload=4000,window=(8,18),relaxed_window=(12,15),eff=.8):
    """
...
#    model.base_load_constraint1 = Constraint(model.T, rule=base_load_constraint1)
    if relaxed_baseload == 0:
        def base_load_constraint0(model, t):
            index= t-1            
            if df.iloc[index].hour in list(range(window[0], window[1])):
                return (model.D[t] + model.Solar[t] + model.Eout[t] - model.Ein[t]) == strict_baseload
#            elif df.iloc[index].hour == window[-1]+1:
#                return Constraint.Skip
            else:
                return (model.D[t] + model.Solar[t] + model.Eout[t] - model.Ein[t]) == 0
        
        model.base_load_constraint0 = Constraint(model.T, rule=base_load_constraint0)

    else:   
        def base_load_constraint1(model, t):
            index= t-1
            
            if df.iloc[index].hour in list(range(window[0], relaxed_window[0])):
                return (model.D[t] + model.Solar[t] + model.Eout[t] - model.Ein[t]) == strict_baseload
            elif df.iloc[index].hour in list(range(relaxed_window[0], relaxed_window[1])):
                return ((model.D[t] + model.Solar[t] + model.Eout[t] - model.Ein[t]) >= strict_baseload) 
            elif df.iloc[index].hour in list(range(relaxed_window[1], window[1])):  
                return (model.D[t] + model.Solar[t] + model.Eout[t] - model.Ein[t]) == strict_baseload
#            elif df.iloc[index].hour == window[-1]+1:
#                return Constraint.Skip
            else:
        ...
        def base_load_constraint2(model, t):
            index = t-1
            
            if df.iloc[index].hour in list(range(window[0], relaxed_window[0])):
                return (model.D[t] + model.Solar[t] + model.Eout[t] - model.Ein[t]) == strict_baseload
            elif df.iloc[index].hour in list(range(relaxed_window[0], relaxed_window[1])):
                return ((model.D[t] + model.Solar[t] + model.Eout[t] - model.Ein[t]) <= relaxed_baseload) 
            elif df.iloc[index].hour in list(range(relaxed_window[1], window[1])):  
                return (model.D[t] + model.Solar[t] + model.Eout[t] - model.Ein[t]) == strict_baseload
#            elif df.iloc[index].hour == window[-1]+1:
#                return Constraint.Skip
        ...
        model.base_load_constraint2 = Constraint(model.T, rule=base_load_constraint2)
    ...
    diesel = [model.D[i].value for i in intervals]
    solar = [model.Solar[i] for i in intervals]
    system_out = [(model.D[i].value + model.Solar[i] + model.Eout[i].value - model.Ein[i].value) for i in intervals]
    df_dict = dict(
        intervals=intervals,
        solar=solar,
        diesel=diesel,
